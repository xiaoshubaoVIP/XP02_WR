...
void Polling(void)
{
	...
	if(Ticks > 0)
	{
		...
		if(TimerInfo[eTmrHdlRFTimeout].TimerFlag & eTmrTimeOutFlag) 	{
			...
			PollingRfTimeout(TimerInfo[eTmrHdlRFTimeout].pCtx);
		}
		...
		if(TimerInfo[eTmrHdlSetLocation].TimerFlag & eTmrTimeOutFlag)
		{
			...
			PollingSetLocation(TimerInfo[eTmrHdlSetLocation].pCtx);
			TmrDeleteTimer(eTmrHdlSetLocation);
		}

		if(TimerInfo[eTmrHdlSetLocationDisplay].TimerFlag & eTmrTimeOutFlag)
		{
			CLEAN_TMR_FLAG(TimerInfo[eTmrHdlSetLocationDisplay].TimerFlag, eTmrTimeOutFlag);
			PollingSetLocationDisplay();
		}
#endif
	}
}
