...
uint16_t CoAlarmCheck(void)
{
	...
	co_ppm = CoGetPpm(CoSampleVoltage,SysAO.TempData);
	...
	if(SysAO.DebugOnFlag == TRUE)
	{
		printf("Co:%d(adc),",CoSample);
		printf("%d(mV),", CoSampleVoltage);
		printf("ppm=%d\n", co_ppm);
	}
...
bool LoadCOData(void)
{
	...
	else
	{
		...
		{
			COAO.CoCariStep = COAO.CoCariStep+1;
		}
		printf("Cail Step = %d\n", COAO.CoCariStep);
	}
...
{ 
	...
	printf("sample=%dmV\n",sample);
#endif
	if(((fault_sample>SENSOR_SHORT_SAMPLE_MIN)
	...
	else if((fault_sample>SENSOR_OPEN_FAULT_SAMPLE) && (SysAO.TempData<74))
	{
...
void SpeakCoAlarmVoice(uint8_t Location)
{
	...
	{
		SpeakerSendCmd(CMD_CO_ALARM_NO_LOCATION);
	}
	else
	{
...
void CoCaliMain(void)
{
	...
	static uint16_t CailCoVolValue = 0;
	uint16_t BatVol = 0;
	...
	switch(SysAO.sig)
	{
		...
		case eEventSigCoCailTimeout:
			...
			printf("Step=%d  ",COAO.CoCariStep);
			printf("Cnt=%d\n", Delay_Cnt);
			printf("Vol=%dmV\n",CoAdcVoltage);
#endif

			Delay_Cnt++;
			if(COAO.CoCariStep==2)//AQ	Step 2 Caribration 150 PPM
			{
				CailStepDisplay(eCail_Co_Step2);
				if(Delay_Cnt>CAIL_CO_WAIT_TIME)
				{
					CailCoVolValue+=CoAdcVoltage;
					if(Delay_Cnt>=(CAIL_CO_WAIT_TIME+SAMPLE_CO_TIME_CNT))
					{
						...
						temp_slope = (CailCoVolValue - (uint16_t)temp_offset)/15; 	// 10ppm
			...
			else if(COAO.CoCariStep==3)//Step 3 Caribration 400 PPM
			{
				...
				if(Delay_Cnt>CAIL_CO_WAIT_TIME)//13??*30S==390S
				{
					...
					if(Delay_Cnt>=(CAIL_CO_WAIT_TIME+SAMPLE_CO_TIME_CNT))
					{
						Delay_Cnt=0;
						CailCoVolValue>>=4;
						ppm = CoGetPpm(CailCoVolValue, SysAO.TempData);
			...
			else //Step 0 Caribration 0 PPM
			{
				...
				if(Delay_Cnt>CAIL_CO_WAIT_TIME)
				{
					...
					if(Delay_Cnt>=(CAIL_CO_WAIT_TIME+SAMPLE_CO_TIME_CNT))
					{
						Delay_Cnt=0;
						CailCoVolValue >>=4;	
						COAO.ppm_caribration_offset = (uint16_t)((uint32_t)CailCoVolValue*1000/co_temp_offset_rate[SysAO.TempData]);
...
#endif
			TmrInsertTimer(eTmrHdlCo, TMR_PERIOD(500), (void *)eCoExitCurrentMode);
			break;
...
{
	static uint8_t TimeoutCnt = 0;
	static uint8_t CheckCotimeCnt = 0;
	...
	switch(SysAO.sig)
	{
		...
		case eEventSigCoAlarmCancel:
			...
			TmrInsertTimer(eTmrHdlCo, TMR_PERIOD(1000), (void *)eCoExitAlarm);
			break;
...
void CoMuteMain(void)
{
	...
	static bool MuteTimeoutFlag = FALSE;
	...
	switch(SysAO.sig)
	{
		...
		case eEventSigCoMuteTimeout:
			...
			else if(MuteTimeCnt == CO_MUTE_TIMEOUT_ReCheck)		//after 6min Check PPM,if PPM>30PPM Alarming
			{
				PPM=CoAlarmCheck();
...
#endif
			TmrInsertTimer(eTmrHdlCo, TMR_PERIOD(1000), (void *)eCoExitMute);
			break;
